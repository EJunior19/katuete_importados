  ...
  <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
    ...
    {{-- Actividad reciente (ventas/compras/pagos) --}}
    <div class="bg-slate-900 border border-slate-700 rounded-xl p-4">
  ...
  document.querySelectorAll('button[data-range]').forEach(b=>{
    b.addEventListener('click', ()=> rango(b));
  });
  ...
  try {
    const res = await fetch(`{{ route('finance.stats') }}?${params.toString()}`, {
    ...
    const j = await res.json();
      ...
      const cxc = j.cxc_estados || {}; // {pagado: n, vigente: n, vencido: n}
